...
import { IUser } from "../LMS/src/users/user.model";
...
function sanitizeField(fields: Array<string>) {
...
const generateSelectSql = <Model>(
  ...
  const sanitaisedFields = sanitizeField(fieldsToSelect as string[]);
  const selectClause = sanitaisedFields.length
    ? sanitaisedFields.join(", ")
    : "*";
  const whereClause = generateWhereClauseSql<Model>(where);
